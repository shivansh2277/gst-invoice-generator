...
def _to_gst_json(invoice: Invoice) -> dict:
    return {
        "invoice": {
            ...
            "logo_base64": invoice.logo_base64,
...
def update_invoice(
    ...
    invoice.logo_base64 = payload.logo_base64
...
def print_invoice_html(invoice_id: int, db: Session = Depends(get_db), _: User = Depends(get_current_user)) -> str:
    ...
    <h4>Terms & Conditions</h4><p>{invoice.terms_conditions or '-'}</p>
    <div style='margin-top:40px;text-align:right'>For {invoice.seller.name}<br/><br/>____________________<br/>{invoice.signature_name}</div>
    </body></html>
    """
